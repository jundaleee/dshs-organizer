...
- **홈**: 오늘 할 일(학교 복습 / 개인 학습 구분해서 미리보기) · 임박한 과제 전체 · 오늘 시간표 · 최근 공지·자료
...
- **할 일 탭 맨 위**: 전체 선생님 로스터를 보고 학교 복습(특정 선생님과 연결) · 자율 학습 · **개인 학습**(특정 수업과 무관한 것)으로 분류 — 어느 과목인지 애매하면 개인 학습으로 들어가서 오분류를 피함
...
```json
{
  ...
  "personal": { "tasks": [ { "id": "", "text": "", "createdAt": 0 } ], "completed": [] },
  "dailyCapacity": 5,
